...
describe("Send password mail use case", () => {
    beforeEach(() => {
        ...
        sendPasswordMailUseCase = new SendPasswordMailUseCase(usersRepositoryInMemory, userTokensRepository, dateProvider, mailProvider)
    })
    ...
    it("Shouldn't be able to send an email to a nonexistent user", async () => {
        await expect(sendPasswordMailUseCase.execute('some@email.com')).rejects.toEqual(new AppError("This user doesn't exist!"))
    })
