...
import { Prisma, Rentals } from '@prisma/client';
...
class RentsRepository implements IRentsRepository {
    private rents: Prisma.RentalsDelegate<false>;

    constructor() {
        this.rents = client.rentals;
    }
    async create({
        user_id,
        car_id,
    ...
    async findOpenRentByCar(car_id: string): Promise<Rent | Rentals> {
    ...
    async findOpenRentByUser(user_id: string): Promise<Rent | Rentals> {
        const rent = await this.rents.findFirst({
            where: {
                user_id,
                end_date: null
    ...
    async findById(rent_id: string): Promise<Rent | Rentals> {
    ...
    async findByUser(user_id: string): Promise<Rent[] | Rentals[]> {
